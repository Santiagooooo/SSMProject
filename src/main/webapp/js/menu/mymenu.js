...
/*$(function() {
	$(".menuBar").hover(function() {
		$(this).css("background", "#15B680");
	}, function() {
		$(this).css("background", "#15B69A");
	});
});*/
...
function Q6MenuBar(objName, barName) {
	...
	this.colors[4] = {
		...
		itemColor : "#555",
		menuBarColor : "#FFFFFF",
		menuContentColor : "#555"
	};
	this.font = {};
	this.menus = [];
	// <div class="title" name="title"><span class="titleText">' + barName +
	// '</span></div><div>
}

/*
 * 添加方法 首先查看this.menus[menuName]这个菜单（就是一个数组）是否存在。 如果不存在，先创建这个菜单（数组）。
 * 使用menuItemName和url创建MenuItem对象，把MenuItem对象添加到菜单（数组）尾部。 frameName --
 * 指定在哪个帧中打开页面
 */
...
/*
 * MenuBar的toString()方法 该方法会生成与MenuBar相关的HTML代码，然后遍历menus属性，生成每个菜单对应HTML代码。
 */
Q6MenuBar.prototype.toString = function() {
	// menuBar对应的<div>
	var str = '<div style="border:1px solid'
			+ this.colors[this.colorStyle].menuBorderColor + ';color:'
			+ this.colors[this.colorStyle].menuBarColor
			+ ';" class = "menuBar" name="menuBar" onClick="' + this.obj
			+ '.showMenu(event, this)">\n';
	// menuBar的标题<div>
	str += '<div style="background-color:'
			+ this.colors[this.colorStyle].menuBgColor
			+ ';" class="barTitle" name="barTitle"><span class="barTitleText">'
			+ this.barName + '</span></div>\n';
	// 使用循环添加每个菜单对应的HTML代码。
	...
	str += '<div style="background-color:'
			+ this.colors[this.colorStyle].menuBgColor
			+ ';" name="barBottom" class="barBottom">';
	str += '</div>';
	return str;
}

/*
 * 该方法首先生成与菜单对应的HTML代码。 menuName是菜单名字，可以使用menus[menuName]来获取一维数组。
...
Q6MenuBar.prototype.menu2Str = function(menuName) {
	...
	str += '<div style="background-color:'
			+ this.colors[this.colorStyle].menuBgColor
			+ ';border-right-color:'
			+ this.colors[this.colorStyle].menuBorderColor
			+ ';border-bottom-color:'
			+ this.colors[this.colorStyle].menuBorderColor
			+ ';" name="menuTitle" class="menuTitle"><span name="menuTitleIcon" class="menuTitleIcon"><img src="'
			+ icon + '"/></span><span class="menuTitleText">' + menuName
			+ '</span></div>\n';
	// 菜单内容对象的<div>，所有菜单荐都存放在这个<div>中。
	str += '<div style="color:' + this.colors[this.colorStyle].menuContentColor
			+ ';" class="menuContent"  name="menuContent">\n';
...
/*
 * menuItem是MenuItem类型的对象。 该方法生成menuItem对应的HTML代码。
 */
Q6MenuBar.prototype.item2Str = function(menuItem) {
	return '<div style="border:1px solid '
			+ this.colors[this.colorStyle].itemBorderColor
			+ ';background-color:' + this.colors[this.colorStyle].itemBgColor
			+ '; color:' + this.colors[this.colorStyle].itemColor
			+ ';" class="menuItem" onMouseMove="' + this.obj
			+ '.itemMouseMove(this)" onMouseOut="' + this.obj
			+ '.itemMouseOut(this)" onClick="skip(\'' + menuItem.url + '\', \''
			+ menuItem.frameName + '\')">' + menuItem.menuItemName + '</div>\n';
}
...
/*
 * 获取当前被点击的menu元素。 只有点击：div menuTitle、img、menuTitleText，才获取父元素。
...
Q6MenuBar.prototype.closeMenu = function(menuBar) {
	...
	for (var i = 0; i < menus.length; i++) {
		if (this.attr(menus[i], 'name') != 'menu')
			continue;
		var childs = menus[i].childNodes;
		// 获取当前menu中所有content
		for (var j = 0; j < childs.length; j++) {
			if (this.attr(childs[j], 'name') != 'menuContent')
				continue;
...
Q6MenuBar.prototype.itemMouseMove = function(e) {
	e.style.border = "1px solid "
			+ this.colors[this.colorStyle].itemBorderMoveColor;
...
/*Q6MenuBar.prototype.menuMouseMove = function(e) {
	e.style.backgroundColor = this.colors[this.colorStyle].menuBgMoveColor;
}

Q6MenuBar.prototype.menuMouseOut = function(e) {
	e.style.backgroundColor = this.colors[this.colorStyle].menuBgColor;
}*/
